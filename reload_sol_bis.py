...
def main():
    ...
    num = 70
    filename = f"/models/athlete_{num}_deleva.bioMod"
    masses = pd.read_csv(CURRENT_DIR+ "/models/masses.csv")
    masse = masses["total_mass"][num-1]
    # mode = "retroversion"
    #mode = "anteversion"
    mode=""
    
    file_path = CURRENT_DIR + f"/results/Sol_athlete{num}_complet_{mode}.pkl"
    ...
    sol.ocp = prepare_ocp(biorbd_model_path=CURRENT_DIR + filename, final_time=(1, 0.5, 1), n_shooting=n_shooting,
                      ...
                      coef_fig=10, mode=mode, n_threads=32, use_sx=False)
